...
func (s *Storage) Init() error {
	...
	for _, statement := range []string{
		...
		s.dbh.Exec(statement)
	}
	// create the data structures if necessary
	for _, query := range []string{
		"CREATE TABLE IF NOT EXISTS visited (id INTEGER PRIMARY KEY, requestID INTEGER, visited INT)",
		"CREATE INDEX IF NOT EXISTS idx_visited ON visited (requestID)",
		"CREATE TABLE IF NOT EXISTS cookies (id INTEGER PRIMARY KEY, host TEXT, cookies TEXT)",
		"CREATE INDEX IF NOT EXISTS idx_cookies ON cookies (host)",
		"CREATE TABLE IF NOT EXISTS queue (id INTEGER PRIMARY KEY, data BLOB)",
	} {
		statement, err := s.dbh.Prepare(query)
		if err != nil {
			return err
		}
